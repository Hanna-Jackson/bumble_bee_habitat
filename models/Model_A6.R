...
## occuring by commenting out lines and adding parameters for the
## region effect. This is to make it easier to comapre which
## parameters have been removed from which models.
...
my.model <- function() {
  ...
  psi.region                  ~ dnorm(0,0.001) ## Added for this model
  ...
  for(site in 1:nsite) {
    ...
    for(species in 1:nspecies){
      ...
      logit(psi[site, species]) <- psi.0                        +
        ...
        psi.region                  * region             [site] + ## Added for this model
...
## Specify the parameters that JAGS will store the values of## parameters to track
my.params <- c('p.0',
               'p.sitetype',
               'p.hoursout', 
               'p.trapjulian',
               ...
               'psi.sitetype',
               'psi.canopyopenness',
               'psi.region',             ## Added for this model
